...
/**
 * Bottom-sheet d'edition d'une carte (titre + description).
 * Pre-rempli a l'ouverture avec les valeurs courantes de la carte.
 *
 * @param {Object} props
 * @param {boolean} props.open
 * @param {() => void} props.onClose
 * @param {Object|null} props.card  carte a editer (objet enrichi useCard)
 * @param {(patch: { title: string, description: string|null }) => Promise} props.onSave
 * @param {string} [props.accentColor='brique']
 */
export default function EditCardModal({ open, onClose, card, onSave, accentColor = 'brique' }) {
  ...
  const canSubmit = trimmedTitle.length > 0 && !unchanged && !submitting
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error('[EditCardModal] submit error:', err)
