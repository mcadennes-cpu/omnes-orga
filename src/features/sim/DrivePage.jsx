...
import { getSimImageUrl } from './simStorage'
...
// ─── Breadcrumb sticky ─────────────────────────────────────────────────────
// subtitle : ligne secondaire optionnelle affichee sous le dernier segment du
// fil (utilise en racine SIM pour "Societe d'Intendance Medicale").
function Breadcrumb({ trail, subtitle, onBack, onCrumb }) {
  return (
    ...
    >
      ...
      {subtitle && (
        <p className="pl-10 mt-0.5 font-sans text-xs text-muted truncate">
          {subtitle}
        </p>
      )}
...
// ─── Toolbar ───────────────────────────────────────────────────────────────
// accent : couleur du CTA "Importer" (olive pour SIM). Defaut marine.
function DriveActions({ canWrite, accent = '#1C3D52', onNewFolder, onUpload }) {
  if (!canWrite) {
    return (
  ...
  return (
    <div className="px-4 pt-3.5 flex gap-2">
      ...
      <button
        type="button"
        onClick={onUpload}
        className="flex-1 h-10 px-3.5 rounded-xl text-white text-sm font-semibold inline-flex items-center justify-center gap-1.5"
...
// ─── Ligne dossier ──────────────────────────────────────────────────────────
function FolderRow({ name, count, meta, accent = '#1C3D52', canMenu, showMeta = true, onOpen, onMenu }) {
...
// ─── Ligne fichier ──────────────────────────────────────────────────────────
function FileRow({ name, author, when, meta, canMenu, showMeta = true, onOpen, onMenu, onDownload }) {
...
// ─── Etat dossier vide ──────────────────────────────────────────────────────
function DriveEmpty({ canWrite, accent = '#1C3D52', onUpload }) {
  return (
    <div className="mx-4 mt-6 px-5 py-9 bg-carte border-[1.5px] border-dashed border-border rounded-card text-center">
      ...
      {canWrite && onUpload && (
        <button
          type="button"
          onClick={onUpload}
          className="mt-4 h-10 px-3.5 rounded-xl text-white text-sm font-semibold inline-flex items-center justify-center gap-1.5"
          style={{ backgroundColor: accent }}
        >
          <Upload size={15} strokeWidth={2} />
          Importer un fichier
...
//
// Adapte de features/cabinet/DrivePage pour le module SIM. Differences :
// - prop "subtitle" : ligne secondaire sous le titre (racine SIM).
// - prop "accent" : teinte le CTA Importer (olive pour SIM).
// - recherche SCOPEE au dossier courant (le filtrage est fait par le parent
//   via filterByTerm ; ici "isSearching" indique juste que le champ est rempli).
// - droits par element : chaque dossier/fichier expose "canMenu" (l'icone "..."
//   ne s'affiche que si une action est possible sur cet element). Calcule par
//   le parent via canEditSim / canDeleteSim.
export default function DrivePage({
  ...
  return (
    <>
      <Breadcrumb trail={trail} subtitle={subtitle} onBack={onBack} onCrumb={onCrumb} />
      <DriveActions canWrite={canWrite} accent={accent} onNewFolder={onNewFolder} onUpload={onUpload} />

      {isEmpty ? (
        <DriveEmpty canWrite={canWrite} accent={accent} onUpload={onUpload} />
      ) : (
        <>
                  ...
                  <div className="border-b border-border">
                    {folders.map((f) => (
                      <FolderRow
                        ...
                        onMenu={() => onMenuFolder && onMenuFolder(f.id, f.name, f.auteurId)}
                      />
                    ))}
                  </div>
                </>
                  ...
                  <div className="border-b border-border">
                    {files.map((f) => (
                      <FileRow
                        ...
                        onMenu={() => onMenuFile && onMenuFile(f.id, f.name, f.mimeType, f.auteurId)}
                      />
                    ))}
                  </div>
                </>
