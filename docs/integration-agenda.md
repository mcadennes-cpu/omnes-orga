...
- ⏳ **Étape 8 — PRÉPARATION COMMENCÉE (26/08/2026).**
  - ⏳ **8E — Mise en production du code — COMMENCÉE (17/09/2026).** Jamais planifiée en détail jusque-là : la branche avait 90 commits d'avance sur `main`, et l'appli déployée n'avait aucune entrée agenda.
    ...
    - **Plan arrêté le 17/09/2026** :
      ...
      - **8F** — scripts du soir, écrits et simulés, non exécutés : `23-10` (6C-4) ; `23-11` (ouverture, avec son retour arrière) ; `23-12` (preuve du mot de passe court sur comptes jetables — celui-ci exécuté avant) ; suites sécurisées pour la production.
      - **8G** — fusion vers `main`, c'est-à-dire déploiement en production. Accord explicite de Matthieu requis.
      - **Soir J** — annonce ; gel de Bolt pour les humains ; 8A-1 sans `--go` ; 8A-1 `--go` ; **8A-1 sans `--go` à nouveau, delta vide** ; `23-6` puis `23-10` ; `23-5` et le conflit du 24/04 ; `23-11` ; test de connexion d'un vrai remplaçant ; gel de Bolt côté données ; suites et doc.
      - **Après J** — remplacer le drapeau bêta par un contrôle de rôle aux quatre endroits.
      - **Décisions ouvertes** : ouverture en posant le drapeau pour les quatre rôles médecins (recommandé : soir J sans déploiement, SQL réversible) ou en retirant la condition ; Airelle SAUVAGE, qui verra la tuile dès le déploiement ; Essai DUPONT ; temps réel ; mode d'extinction de Bolt (proposé : page de redirection, puis pause du projet Supabase, réversible).
