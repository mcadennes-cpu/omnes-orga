...
- ✓ **Étape 6 / MOD-2 — FAITE (03-26/08/2026).** Arbitrée le 03/08 ; MOD2-A à MOD2-C livrées le 06/08, MOD2-D et MOD2-E le 24/08, MOD2-F les 24-26/08 — **toutes validées en usage réel par Matthieu** au fil des sous-étapes. **MOD2-G est abandonné** (décision du 24/08, après avoir eu le bandeau en main : il porte déjà un « Annuler » visible, un raccourci clavier n'ajouterait qu'un second chemin vers le même geste, sur des écrans où l'on saisit aussi du texte). Les **six** suites de test totalisent **110 contrôles**.
  ...
  - ⚠ **À remettre : le créneau « J6 Beaune »**, supprimé par Matthieu le 24/08/2026 en validant l'écran refait par MOD2-F-2 (suppression volontaire, sur la copie de travail). Script prêt, **non exécuté** : `docs/sql/23-2-agenda-restaure-j6-beaune.sql`, reconstruit depuis `22-6A-2` (ligne 52). Seul le `default_room_id` est **déduit** et non retrouvé (bloc retirable). La suppression n'a emporté ni garde ni règle de roulement — les deux verrous l'interdisaient (0 garde, 0 règle mesurées avant). **C'est cet incident qui a produit MOD2-F-4** : au moment des faits, la suppression n'avait laissé aucune trace.
  - ⚠ **Point ouvert, hors MOD-2 — `is_agenda_coordinator` exclut d'être médecin.** Relevé par Matthieu le 26/08 en cherchant à tester les demandes : il ne se voit pas dans la liste des médecins à qui attribuer une garde. `AssignDoctorModal` filtre sur `.eq('role', 'doctor')`, or la vue `agenda.profiles` calcule `role` avec `case when is_agenda_coordinator then 'coordinator' else 'doctor' end` — **être coordinateur exclut donc mécaniquement d'être médecin**. Or Matthieu est l'un des 9 associés du roulement (`MC`) : **156 gardes attribuées et 55 règles de roulement** à son nom, mais impossible de lui en attribuer une à la main. Pour Charlotte (0 garde, 0 règle) le comportement actuel est juste. C'est la moitié manquante du découplage entamé en 7A : `is_agenda_coordinator` a bien détaché « coordinateur » du rôle applicatif, mais le module lit encore un rôle **unique** là où Matthieu cumule deux casquettes — et le rôle Orga ne peut pas servir de rattrapage (tous deux `super_admin`). **Correctif cohérent** : une colonne explicite `is_agenda_doctor`, sur le modèle de `is_agenda_coordinator`, plutôt qu'une déduction. **À traiter avant l'étape 8** — sans quoi le seul coordinateur qui exerce est inassignable.
