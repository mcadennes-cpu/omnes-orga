...
- ⏳ **Étape 6 / MOD-2 — EN COURS.** Arbitrée le 03/08/2026 ; MOD2-A à MOD2-C livrées le 06/08/2026, MOD2-D et MOD2-E le 24/08/2026 — **toutes validées en usage réel par Matthieu** au fil des sous-étapes. **MOD2-G est abandonné** (décision de Matthieu du 24/08/2026, après avoir eu le bandeau en main : il porte déjà un « Annuler » visible, un raccourci clavier n'ajouterait qu'un second chemin vers le même geste, sur des écrans où l'on saisit aussi du texte). MOD-2 se referme donc sur **MOD2-F**, en cours : F-1 (vocabulaire) et F-2 (écrans de paramètres) livrées et validées le 24/08.
  - ⚠ **À remettre : le créneau « J6 Beaune »**, supprimé par Matthieu le 24/08/2026 en validant l'écran refait par MOD2-F-2 (suppression volontaire, sur la copie de travail). **Aucune trace en base** : les déclencheurs de journal de MOD2-A portent sur `shifts`, `requests`, `fixed_duty_series` et `rotation_plans`, **pas** sur `shift_types` / `sites` / `rooms` — ni le journal ni `restaurer_action` ne peuvent rien ici. Script prêt, non exécuté : `docs/sql/23-2-agenda-restaure-j6-beaune.sql`, reconstruit depuis `22-6A-2` (ligne 52). Seul le `default_room_id` est déduit et non retrouvé (bloc retirable). La suppression n'a emporté ni garde ni règle de roulement — les deux verrous l'interdisaient (0 garde, 0 règle mesurées avant).
