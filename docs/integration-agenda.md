...
## Accès bêta (module caché) — ~~plan d'origine~~, terminé le 22/09/2026

> **Section historique.** La phase bêta est close : l'accès au Planning tient
> au **rôle** depuis le 22/09/2026 (sous-étape 8N, script `23-25`). Ce qui
> suit décrit le plan de juillet, conservé pour comprendre l'historique des
> scripts `22-2A`, `23-7` et `23-12`. **Ne pas appliquer.**
...
⚠ **Ce dernier conseil était un piège, et c'est ainsi qu'il a été traité en
8N.** « Supprimer la condition » aurait ouvert le planning entier au compte
`poste_bureau` — la borne partagée du cabinet, active, que seul le drapeau
tenait à l'écart (mesuré : 2511 gardes). La condition a donc été **remplacée**
par une liste blanche de rôles, pas supprimée. La colonne, elle, subsiste et
reste remplie selon le rôle par le déclencheur de `23-14` ; plus personne ne
la lit pour décider.
...
- ✓ **Étape 2 — FAITE (23/07/2026)** — branche `feature/module-agenda`.
  ...
  - **2B — Tuile conditionnelle** : entrée `agenda` dans `MODULES` (`src/lib/modules.js`) — label affiché **« Planning »** (le nom que le cabinet utilise déjà ; l'identifiant interne reste `agenda` partout : dossier du module, colonne SQL, docs), icône `CalendarClock`, couleur `canard`, `poste_bureau` exclu des rôles. Mécanisme générique `betaFlag: 'agenda_beta_access'` : `getVisibleModules(role, profile)` masquait tout module dont le flag du profil était faux. **Retiré le 22/09/2026 (8N)** — le drapeau *et* le mécanisme générique, plus aucun module ne s'en servant : la tuile ne dépend plus que de `allowedRoles`, et `getVisibleModules(role)` n'a plus besoin du profil.
  - **2C — Route** : `/planning` (alignée sur le label plutôt que sur l'identifiant interne), page `src/pages/Agenda.jsx` dans le bloc `ProtectedRoute`. Garde d'accès : redirection vers l'accueil, **après** la fin du chargement du profil — sur `agenda_beta_access` jusqu'au 22/09/2026, sur `canAccessAgenda(role)` depuis (8N) (sinon un F5 sur `/planning` éjecterait un utilisateur légitime pendant le fetch). Module chargé via `React.lazy()` + `Suspense` → chunk séparé (~223 kB, 42 kB gzip) téléchargé uniquement à l'ouverture de la route ; le module entre enfin dans le graphe d'imports de `vite build` (102 modules transformés).
...
- ⏳ **Étape 8 — BASCULE FAITE LE 17/09/2026 (ouverture à 14h00)** — préparation commencée le 26/08/2026. Bolt archivé et mis en pause à 14h19. Reste : l'annonce au cabinet, puis les chantiers « après J ».
  - ✓ **8Q — Page de redirection à la place de l'ancienne appli Bolt — FAITE (24/09/2026).** Chantier 4 de la liste d'après J.
    - **Le constat.** Depuis la mise en pause du projet Supabase de Bolt (17/09), `omnes-planning-share-6kyv.bolt.host` affichait une appli qui tournait dans le vide en cherchant sa base. Quiconque avait gardé l'ancien favori tombait sur un écran cassé, sans indication.
    - **Choix de Matthieu : une page d'explication avec bouton**, plutôt qu'une redirection automatique. La personne comprend ce qui a changé et peut mettre à jour son favori, alors qu'une redirection silencieuse l'aurait laissée indéfiniment sur l'ancienne adresse.
    - **La page** — source versionnée dans `docs/bolt-redirection/index.html`, collée par Matthieu à la place de l'`index.html` du projet Bolt.new, puis publiée. **100 % statique** : ni script, ni appel à Supabase, elle ne peut donc pas tomber en panne comme l'ancienne appli. Couleurs et typo du design system recopiées en CSS (pas de Tailwind dans ce fichier). Bouton « Ouvrir le Planning » vers `https://omnes-orga.vercel.app/planning`, puis une aide à la connexion et au favori.
    - **Ce que la page ne dit pas, volontairement : « même mot de passe ».** C'est vrai pour les remplaçants (empreintes recopiées en 8C), mais pas forcément pour les associés, qui avaient déjà leur compte Orga. Elle renvoie donc vers « Mot de passe oublié ? ».
    - **Réversible.** Le code de l'ancienne appli est resté dans le projet Bolt, simplement plus chargé : remettre l'ancien `index.html` suffit.
    - **Vérifié en ligne après publication.** Contenu servi identique au fichier du dépôt, à une ligne près : le badge `bolt.new/badge.js` que l'hébergeur Bolt injecte de lui-même, hors du code (inoffensif, il n'appelle ni Supabase ni l'ancienne appli). Logo et lien cible répondent. L'ancien site n'avait **pas de service worker** (`/sw.js` n'était que la page par défaut) : aucun téléphone ne garde l'ancienne appli en cache au-delà du cache HTTP de 4 h de l'hébergeur.
  - ✓ **8P — Refonte visuelle de « Planning du jour » et « Mes gardes », préparée avec Claude Design — FAITE (24/09/2026), P-A à P-C.** Commits `eade63c` (P-A), `b040aab` (P-B), `f973cd6` (P-C). Suites rejouées après chaque sous-étape : `test:fuseaux` **167/167**, `test:pagination` **55/55**. Chaque sous-étape a été validée par Matthieu sur son téléphone (serveur de dev exposé sur le réseau local, port 5175).
    - **La méthode, réutilisable pour les autres écrans.** Matthieu a retravaillé l'écran dans Claude Design, à partir d'un prompt qui décrit **uniquement les briques existantes** : palette, classes typo composées, radii, composants du module (`BottomSheet`, `Segmented`, `Pill`, `Avatar`…), en exigeant que tout écart au design system soit listé à part plutôt qu'appliqué en silence. Le prompt est versionné dans `docs/design/prompt-claude-design-planning.md` et tenu à jour de ce qui est livré. Le code produit n'est **pas collé tel quel** : il est retranscrit dans le vrai composant, avec ses données, son temps réel et ses états de chargement et d'écran vide, que la maquette ignorait.
    - **Ce que la relecture de la maquette a corrigé.** (1) Elle **recodait la règle des couleurs**, en se trompant : J5 Dijon (`12:00-20:00`, 46 gardes à venir) passait en marine « nuit ». On garde `resolveHoraire()`, réglée et testée en 8D ; `HORAIRE_STYLES` gagne seulement `bgClass` et `textClass`. (2) Elle **inventait les noms de créneaux** : « J2 » y valait `08:00-18:30`, alors qu'en base J2 est `14:00-22:00` et que `08:00-18:30` correspond à J3, J4, J7 ou J8. Les vrais noms sont lus par jointure sur `shift_type_id` (mesuré : **748/748** gardes assignées à venir reliées à leur créneau, zéro écart entre `shifts.shift_type` et `shift_types.time_range`). (3) Son README invoquait une « info férié existante » : **le module ne connaît pas les jours fériés**, le week-end reste « samedi et dimanche ». (4) Son ouverture du calendrier par `showPicker()` est remplacée par le vrai champ date, invisible, posé sur l'icône : plus fiable sur iPhone.
    - **P-A — Planning du jour** (`DailyScheduleView.tsx`). Bloc titre et cadre blanc supprimés (l'onglet actif le dit déjà). Date en titre (`text-h1`), nom du jour en `text-eyebrow`, en brique le week-end. Jour précédent / calendrier / jour suivant regroupés à droite en boutons de 44 px. Site choisi par des **chips** au lieu d'une liste déroulante (2 sites en base). Compteur de médecins. **Carte entière teintée** à la couleur du créneau, avatar 72 px sur une colonne blanche de 88 px, horaire en `text-h2`, badge du nom court du créneau. **Tri par horaire puis par nom** (décision de Matthieu ; c'était par nom seul).
    - **Nom court du créneau** — `nomCourtCreneau()` dans `horaireStyles.ts`. Les noms en base sont saisis de façon irrégulière (« J3 Dijon », « WE1 beaune 08h-20h », « pré - J2 Dijon ») : le site et l'horaire éventuel sont retirés **à l'affichage seulement**, la base n'est pas touchée. Joué sur les 19 créneaux en base : J1 à J8, « J5 bis », WE1, WE2, « pré-J2 », sans raté.
    - **P-B — Onglet actif du header en marine** (`AgendaHeader.tsx`), sur tous les onglets et pour les deux rôles, à la demande de Matthieu. **Déviation assumée** : ailleurs dans l'appli, l'élément actif prend la couleur du module. Le canard reste l'accent du Planning partout ailleurs : `Segmented`, chips de site, filtres, filigrane.
    - **P-C — Mes gardes aligné sur Planning du jour** (`MyScheduleView.tsx`). Même carte teintée, même badge, même suppression du bloc titre. À la place de l'avatar (ce serait toujours celui du médecin lui-même), la colonne blanche porte la **date façon page de calendrier** (« JEU / 24 / sept. », jour en brique le week-end). Au passage, deux messages d'erreur retrouvent leurs accents (« déjà assignée », « indisponibilité »).
    - **Le liseré en L de 8D-2 n'est plus affiché nulle part.** Le champ `borderClass` est conservé dans `horaireStyles.ts` le temps de confirmer le choix à l'usage ; à retirer s'il reste inutilisé, comme `bandClass` l'avait été.
    - ⚠ **Incident Supabase pendant la séance (24/09, vers 20h00).** Le service de connexion (`/auth/v1/token`) ne répondait plus : 40 s sans réponse, depuis le terminal comme depuis le navigateur, alors que la lecture des données fonctionnait et que le tableau de bord le déclarait « sain ». Revenu de lui-même quelques minutes plus tard. Aucun lien avec le code : rien n'était déployé. À retenir : **un bouton « Connexion… » qui tourne sans fin peut venir de Supabase et non de l'appli**. Ce diagnostic se fait en quelques secondes avec un `curl` sur `/auth/v1/token`.
  - ✓ **8O — L'export et l'impression du planning tronquaient à 1 000 gardes, en silence — FAITE (22/09/2026).** Chantier 7 de la liste d'après J. Nouveau `src/modules/agenda/lib/pagination.ts`, suite de test `docs/tests/22-8O-test-pagination.ts` (`npm run test:pagination`), commit `00021c4`.
    - **Le défaut.** PostgREST ne renvoie **jamais plus de 1 000 lignes** par requête, et ne le signale pas : la requête réussit, et le code croit avoir tout reçu. `exportUtils.ts` et `printPlanning.ts` lisaient les gardes en une seule requête. Mesure du 22/09 : **2 691 gardes sur l'année 2026**, donc un export d'année produisait un fichier de 1 000 lignes — **37 % du planning** — qui ressemblait en tout point à un export réussi. Le seuil était atteint **dès quatre mois** (~225 gardes par mois).
    - **Déjà connu, jamais corrigé.** La limite avait été rencontrée et traitée en 8J pour la sauvegarde (`23-16`), qui lit par tranches ; la note de l'époque signalait que l'export, lui, ne le faisait pas — « non corrigé ». C'est cette ligne qui se referme ici.
    - **Décision de Matthieu, en séance.** Son premier réflexe était d'afficher un message d'erreur au-delà d'une certaine plage, l'usage réel de Charlotte étant d'un mois. Écarté après examen : un message fiable suppose de compter les gardes d'abord, donc coûte presque autant à écrire que la lecture par tranches, tout en **déplaçant le travail sur l'utilisateur** (douze exports à recoller pour une année) et en reposant sur une hypothèse d'usage qu'un bilan annuel suffirait à casser. Son intuition visait pourtant le bon problème — le **silence** — d'où le compteur ci-dessous.
    - **Le piège de la pagination : le tri doit être TOTAL.** On lit 0-999, puis 1000-1999 ; entre les deux, la base doit trier à l'identique. `order('date')` seul ne suffit pas — toutes les gardes d'une même journée sont à égalité, et PostgreSQL n'est pas tenu de les ranger pareil d'un appel à l'autre. Une garde peut alors changer de tranche, donc apparaître **en double ou disparaître**. Corriger la troncature avec un tri partiel aurait remplacé un oubli visible par un oubli silencieux : les deux appelants terminent désormais leur tri par `id`, et la suite de test vérifie cette règle **par lecture du code**.
    - **Ce qui a été livré.** `lireToutesLesLignes()` rappelle la requête par tranches de 1 000 jusqu'à en recevoir une incomplète — seul signal disponible, PostgREST ne disant pas combien de lignes restent. Garde-fou à 500 tranches (une source qui ne se tarirait jamais remplirait la mémoire du téléphone), et toute erreur remonte **sans rendre de lignes partielles**, ce qui serait le défaut d'origine sous une autre forme. La modale d'export affiche « *N* gardes exportées » au lieu de se fermer sans rien dire.
    - **Vérifié** : 55 contrôles au vert (0, 1, 999, 1 000, 1 001, 2 000, 2 691, 3 000 lignes — compte exact, sans doublon ni trou, et nombre de tranches attendu : 1 000 lignes coûtent **2** requêtes, la seconde servant à constater qu'il n'y a plus rien) ; garde-fou ; remontée d'erreur en 1re et en 3e tranche ; non-régression sur les deux appelants. Build OK. **Export réel d'une année fait par Matthieu dans l'appli.**
    - **Non concerné, vérifié** : les autres lectures de `shifts` du module sont bornées à un jour, une semaine ou un mois, ou ciblées sur un identifiant. « Mon planning » (`MyScheduleView`) filtre sur un médecin et ses gardes à venir — quelques dizaines de lignes.
  - ✓ **8N — Sortie de bêta : le rôle décide, le drapeau `agenda_beta_access` ne décide plus — FAITE (22/09/2026), 1A à 1F.** Premier des chantiers « après J » listés en 8E. Scripts `docs/sql/23-25-agenda-sortie-de-beta.py` (pose et retour arrière) et `docs/sql/23-26-agenda-preuve-sortie-de-beta.py` (preuve, n'écrit jamais). Bascule en production le 22/09 à 11h31, code en `cd4835b`.
    - **Le drapeau n'était plus un confort d'interface.** `22-2A` l'annonçait en juillet comme un simple masquage d'icône — « aucune policy RLS ne dépend de cette colonne ». Ce n'est plus vrai depuis `7C-3` : mesuré le 21/09, les **51** policies du schéma `agenda` passent **toutes** par `agenda.peut_acceder()` (11) ou `agenda.est_coordinateur()` (41), **zéro** n'échappe aux deux — et les deux fonctions testaient `agenda_beta_access`. Le drapeau était la vraie barrière de données du module.
    - **Ce n'était plus une règle, mais un doublon.** Depuis `23-14` (8I, 17/09), le déclencheur `designer_agenda_selon_role` remplit lui-même le drapeau **à partir du rôle**, à la création du compte et à chaque changement de rôle. La règle « le rôle décide » était donc déjà écrite, mais en amont, dans une colonne recopiée, au lieu d'être lue au moment de décider. Le chantier supprime l'intermédiaire.
    - **L'équivalence a été mesurée, pas supposée.** Sur les 41 fiches du 21/09, le drapeau valait exactement « rôle médecin » : **zéro divergence** (2 super_admin, 3 associés gérants, 5 associés actifs, 23 remplaçants actifs le portent ; le poste de bureau et les 7 comptes inactifs ne l'ont pas). `23-25` refait ce contrôle en base avant chaque écriture et s'arrête s'il trouve le moindre écart.
    - **Le piège évité : remplacer la ligne, jamais la supprimer.** `peut_acceder()` portait le commentaire « retirer la ligne ci-dessous pour ouvrir à tous ». Au pied de la lettre, cela donnait l'accès complet au planning au compte `poste_bureau` — la borne partagée du cabinet, active, seule tenue à l'écart par le drapeau. La condition est donc remplacée par une **liste blanche de rôles** (`super_admin`, `associe_gerant`, `associe`, `remplacant`), alignée sur les `allowedRoles` de la tuile : un rôle ajouté plus tard n'aura rien par défaut. C'est l'oubli qui ferme, pas celui qui ouvre.
    - **Ce que la contre-épreuve a montré, et qui justifie le chantier à lui seul.** Avant la bascule, poser `agenda_beta_access = true` sur le compte du poste de bureau lui ouvrait **les 2511 gardes** — mesuré en transaction annulée sur la production le 21/09. Après, le même geste ne donne rien : `acces = false`, zéro garde. Le trou était latent (personne n'avait de raison de poser ce drapeau), du même genre que celui du chantier D : il s'ouvrait au premier geste de quelqu'un voulant bien faire.
    - **La preuve, en trois volets** (`23-26`, jamais d'écriture durable). **A** : les 41 fiches une par une, chacune sous son identité (`set_config('request.jwt.claims', …, true)`, ce que lit `auth.uid()`) — réponse des deux fonctions identique avant et après. **B** : 7 témoins, un par couple (rôle, actif), sous `set local role authenticated`, comptage des lignes visibles sur les 16 tables — **zéro écart sur 112 comptages**. **C** : la contre-épreuve, qui force le drapeau à diverger du rôle pour établir lequel des deux décide — ses réponses s'**inversent** entre avant et après, ce qui est précisément ce qu'un « rien n'a changé » ne prouverait pas.
    - **Retour arrière, joué pour de vrai deux fois sur l'environnement de test.** `23-25` porte le corps d'origine des deux fonctions en dur et vérifie par empreinte md5 (`a06572b0…`, `0ff87d6a…`) que c'est bien lui qu'il remplace — il refuse d'écrire sinon, car il restaurerait alors une version périmée. `--retour-arriere --go` rend le texte d'origine **au caractère près**, commentaire `-- PHASE BETA` compris.
    - **La colonne `profiles.agenda_beta_access` n'est PAS supprimée.** Plus personne ne la lit pour décider, mais elle reste en base et le déclencheur de `23-14` continue de la remplir. Raison : six scripts de `docs/sql` la lisent (dont les suites de test et `23-12`), et un `DROP COLUMN` serait irréversible. Sa suppression, avec la reprise de ces six scripts, reste un chantier ouvert à part.
    - **Côté code (`cd4835b`)** : nouveau `canAccessAgenda(role)` dans `src/lib/permissions.js`, sur le patron de `canAccessSim` et `canAccessCodes` ; `src/pages/Agenda.jsx` teste ce rôle au lieu du drapeau, la garde continuant de rediriger vers l'accueil (comportement inchangé) ; `src/lib/modules.js` perd `betaFlag` **et** le mécanisme générique qui allait avec, plus aucun module ne s'en servant — `getVisibleModules` n'a donc plus besoin du profil, et `src/pages/Home.jsx` ne le lui passe plus. La liste des rôles vit désormais à trois endroits (fonction SQL, `allowedRoles`, `canAccessAgenda`), duplication que le projet pratique déjà pour SIM et les Codes d'accès ; les trois commentaires se citent mutuellement.
    - **Un défaut attrapé par l'environnement de test** : la première version posée y faisait afficher « drapeau PRESENT » après bascule — le commentaire de la nouvelle fonction citait encore `agenda_beta_access`, et la détection l'attrapait. Sans conséquence fonctionnelle, mais l'affichage mentait. Corrigé en reformulant le commentaire et en cherchant désormais la **condition** (`and agenda_beta_access`) plutôt que la mention.
  - ✓ **8M — Le fuseau horaire : le planning se décalait d'un jour hors de métropole — FAITE (21/09/2026), 8M-1 à 8M-8.** Signalé par Matthieu le 21/09 : Thibault Guérin, associé actuellement à Tahiti, voyait sur son téléphone « les horaires d'un samedi (8h-20h) proposés un vendredi ».
    ...
    - ✓ **8M-6 — le roulement lit la vraie date de la garde.** Famille D, 18 remplacements (`AssignDoctorModal` 6, `useShiftDetail` 12). Ces dates alimentent `getPlanForDate`, `getRotationWeek` et `getRotationSlot`, qui décident quelle case du roulement occupe une garde. Mesuré sur les **730 dates de 2026-2027** et les **deux vrais plans** du cabinet (V1 du 29/12/2025, V2 du 04/01/2027, cycles de 8 semaines) : `Europe/Paris` **identique** sur les trois calculs — iso-comportement, aucun risque pour Charlotte ; `Pacific/Tahiti` l'ancien code se trompait de jour de la semaine sur **730 dates / 730**, de semaine de roulement sur **104** (tous les lundis — et c'est justement le lundi qui porte le calcul) et de plan sur **1** (la bascule V1→V2 du 04/01/2027).
    - ✓ **8M-7 — les séries de gardes.** Famille E, la seule qui **écrit en base** : une série « tous les vendredis » pouvait ouvrir des gardes le samedi. Mesuré sur 4 séries réalistes : `Pacific/Tahiti` et `America/Cayenne` **4 fausses sur 4**.
      - ⚠ **Et 1 sur 4 était fausse à Paris aussi**, sans aucun voyageur : celle à cheval sur le passage à l'heure d'été. `setDate` conserve l'heure locale (01h00) ; après le 29/03 à 02h00 Paris passe à UTC+2, 01h00 locale devient 23h00 UTC **de la veille**, et `toISOString` recule d'un jour. Le 29/03 était écrit **deux fois**, le 30/03 **jamais**. C'est exactement pour cela que `depuisJour` place la Date à **midi** : douze heures de marge absorbent le décalage d'une heure.
      - ✓ **Vérifié en base (lecture seule)** : **aucune série existante n'est à cheval sur un passage à l'heure d'été**. Le défaut n'a donc jamais produit de mauvaises données, et il n'y a **rien à réparer**.
      - Au passage, `ExportPlanningModal` compare deux chaînes `'AAAA-MM-JJ'` plutôt que deux `Date` — comportement identique, et le module n'a plus **aucun** `new Date()` posé sur une date de garde.
    - ✓ **8M-8 — la suite de test, installée dans le dépôt.** `docs/tests/22-8M-8-test-fuseaux.ts`, lancée par **`npm run test:fuseaux`** (script ajouté à `package.json` ; aucune dépendance installée — Node 24 exécute le TypeScript nativement). **167 contrôles** : 2 statiques sur le code source, puis 33 rejoués dans chacun des **5 fuseaux**.
      - **Pourquoi plusieurs fuseaux** : le défaut de 8M était invisible depuis Dijon, les deux lectures coïncidant à l'est de Greenwich. Une suite tournant dans le seul fuseau du développeur n'aurait rien vu.
      - **Le test a été éprouvé** : en cassant volontairement `jourLocal`, la suite tombe à **29 échecs** et sort en code 1 — et ce ne sont pas les mêmes échecs selon le fuseau, ce qui valide le fait de tous les rejouer.
      - **Les 2 contrôles statiques** interdisent, dans tout le module, `toISOString()` pour fabriquer un jour et `new Date(<chaîne>)` sur une date de garde, et vérifient qu'il n'existe **qu'une** définition de `jourLocal` — c'est la duplication (trois copies de `formatDateLocal`) qui avait laissé le défaut s'installer.
      - `DeleteWeekTemplateModal` : le paramètre `dateString` devient `horodatage`. Il porte un `created_at`, donc un vrai instant, où `new Date()` est correct. Le renommage lève le faux positif du contrôle statique et dit l'intention **dans le code** plutôt que dans une exception de la règle.
    - **Aucune écriture en base sur 8M** : pas de script `docs/sql`, pas de sauvegarde préalable, pas de migration. Corrections de code seulement, réversibles par `git revert`. **Déployé en production le 21/09/2026** (`main` passé de `ae33ccc` à `f40f05d` pour 8M-1 à 8M-5 ; 8M-6 à 8M-8 suivent).
    - **Bilan** : **48 occurrences corrigées dans 17 fichiers**, 6 familles, une source unique (`lib/dates.ts`) et une suite de test qui interdit la rechute. Deux défauts qui jouaient **en France** ont été trouvés en chemin et corrigés : les 91 gardes du dernier jour du mois (8M-4) et les séries à cheval sur le passage à l'heure d'été (8M-7).
