...
| Email transactionnel | Resend (sandbox `onboarding@resend.dev`, à migrer sur domaine vérifié avant prod) |
...
15. **Étape 15** — Reset password (page "Mot de passe oublié", page "Définir nouveau mot de passe", flux complet via Resend). Prérequis : Resend déjà en place depuis 14D.
...
- **Envoi mail Resend en mode sandbox** — tant qu'aucun domaine n'est vérifié dans Resend, l'envoi mail (création de médecin avec case "Envoyer par email" cochée, et plus tard reset password en étape 15) ne peut atteindre **que** l'adresse `mcadennes@gmail.com` (propriétaire du compte Resend). Toute autre adresse renverra un succès de création de compte mais un échec d'envoi mail, avec un message Resend explicite affiché à l'utilisateur. À lever avant ouverture aux 30 utilisateurs en achetant un domaine (~10 €/an) et en vérifiant 3 enregistrements DNS dans Resend (SPF, DKIM, MX optionnel). Aucune modification de code requise, juste un changement de l'expéditeur `from` dans l'Edge Function `create-medecin`.
...
- **Pas de changement forcé de mot de passe à la première connexion** — décision pragmatique (étape 14). Le mot de passe temporaire reste valide tant que le médecin ne le change pas. Les médecins changeront leur mot de passe quand ils en auront besoin (via le futur reset password de l'étape 15, ou via une future page "Modifier mon mot de passe" dans Profil).
