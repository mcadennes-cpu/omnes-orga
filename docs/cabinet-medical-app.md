...
- **Deep-link partiel des notifications** — si l'app est en arrière-plan sur une **autre** carte au moment du tap, le `notificationclick` fait un `focus()` sans naviguer vers la carte cible. Quand l'app est **fermée**, elle s'ouvre correctement sur la bonne carte (`openWindow`). Amélioration possible : `client.navigate()` dans le service worker.
