...
Depuis l'étape 18, chaque tuile de module (Discussion, Immobilier, Événements) peut porter une pastille indiquant le nombre de choses en attente pour l'utilisateur, et une section « En attente » sous la grille liste ces éléments (messages non lus, sondages à voter, sondages de présence) avec accès direct à la carte ou à l'événement concerné. Les deux sont alimentés par une seule fonction RPC `get_mon_activite()` via le hook `useMonActivite` (cf. plan de développement, étape 18).
...
- `/evenements/:id` — détail : bloc résumé (date complète, lieu, auteur), description, section sondage (si actif), section documents. Menu trois-points Modifier / Supprimer selon les droits, en bottom-sheet via Portal (même pattern que Discussion/Immobilier).
...
- **Export et impression du planning tronqués à 1 000 gardes — RÉSOLU (22/09/2026, 8O)** — PostgREST ne renvoie jamais plus de 1 000 lignes par requête, **sans erreur au-delà** : `exportUtils.ts` et `printPlanning.ts` lisaient les gardes en une seule requête, et un export d'année produisait un fichier de 1 000 lignes sur 2 691 (37 % du planning) qui ressemblait à un export complet. Le seuil tombait dès quatre mois. Corrigé par `src/modules/agenda/lib/pagination.ts` (`lireToutesLesLignes()`), qui rappelle la requête par tranches de 1 000 jusqu'à en recevoir une incomplète. **Règle à ne pas perdre** : toute lecture paginée doit trier sur une colonne **unique** en dernier critère (`id` ici) — sinon une ligne peut changer de tranche entre deux appels et se dupliquer ou disparaître, ce qui remplacerait un oubli visible par un oubli silencieux. Vérifié par `npm run test:pagination` (55 contrôles), qui contrôle aussi cette règle par lecture du code. La sauvegarde `23-16` lisait déjà par tranches depuis 8J ; les autres lectures de `shifts` du module sont bornées à un jour, une semaine ou un mois.
...
- **`src/components/common/Avatar.jsx`** — composant unique pour afficher l'avatar d'un médecin. Si `profile.photo_url` est défini, charge l'URL signée Supabase via cache mémoire et affiche la photo en `object-cover`. Sinon, affiche les initiales sur fond coloré (palette déterministe via `getAvatarPalette`). Pendant le chargement de l'URL signée : on affiche les initiales (pas de flash blanc). Si l'URL signée expire ou que l'image plante (`onError`) : on retombe sur les initiales (fail-safe silencieux). Props : `profile` (objet avec `prenom`, `nom`, `photo_url`), `size` (en pixels), `className`, `alt` (optionnel, pour MemberAvatars où le nom n'est pas affiché à côté). Centralise un pattern d'affichage qui était dupliqué dans 6+ écrans avant l'étape 16E.

- **`src/components/common/AvatarUploadModal.jsx`** — modale bottom-sheet d'upload de photo de profil, en 3 étapes (sélection → crop → confirmation). Utilise `react-easy-crop` pour le crop carré obligatoire, puis `compressToSquareJpeg` pour la compression (JPEG carré 512×512, qualité 0.85 dégradable, max ~500 Ko). Pattern Portal calqué sur les autres modales bottom-sheet du projet (CreateCardModal Immobilier). Branchée dans Profil et MedecinDetail.

- **`src/components/common/ImageViewerModal.jsx`** (étape 19) — visionneuse d'image plein écran via Portal, affichée dans l'application (pas d'ouverture externe). Zoom maison sans dépendance (double-tap, pincer, glisser). Props : `{ open, src, alt, onClose, onDownload }`. Le `onDownload` sert de repli quand une image ne peut être rendue (HEIC sur certains navigateurs). Utilisée pour toutes les images de l'app : pièces jointes Discussion/Immobilier et documents Cabinet/SIM/Événements.

- **`src/components/common/MembersSheet.jsx`** (24/09/2026) — feuille bottom-sheet **en lecture seule** listant tous les participants d'un tableau, ouverte en tapant la pile d'avatars du header (Discussion et Immobilier). Répond au fait que le « +N » masquait les membres au-delà du 4e sans aucun moyen de les voir. Props : `{ open, onClose, profiles, ownerIds, currentUserId, accentColor, title, ownerLabel }`. Affiche avatar, nom, spécialité, marqueur « (vous) » et un badge « Créateur » (composant `Pill`, couleur du module) ; tri créateur d'abord puis alphabétique. **Aucune requête ajoutée** : les profils de tous les participants étaient déjà chargés par les hooks `useBoard`, le « +N » n'était qu'un `slice(0, 4)` visuel.

  Pourquoi une bottom-sheet et pas un menu déroulant : le `<header>` des pages tableau est en `overflow-hidden` (filigrane Omnès), ce qui rogne intégralement tout dropdown CSS ouvert depuis lui — c'est le bug déjà rencontré sur `/evenements/:id` (cf. limitations connues). Le Portal sort du stacking context, comme `BoardActionsMenu`.

  Le badge marque **une seule personne** dans les deux modules : celle qui a créé le tableau. Côté Immobilier, `ownerIds` est un tableau et le code parle d'`owner` au pluriel, mais le seul chemin qui écrit ce rôle est le trigger `add_immobilier_board_creator_as_owner` (10B-1), qui inscrit `auteur_id` ; tous les autres chemins écrivent `'member'` et les RPC sont explicitement anti-promotion (10A-2). L'application ne sait pas promouvoir quelqu'un en owner — seule une intervention en base le permettrait.
...
Depuis l'étape 19, `storageOpen.js` expose aussi `isImage(mimeType)` et `createSignedImageUrl({ supabase, bucket, storagePath, expiresIn })`, utilisés par la visionneuse d'image intégrée. Le hook transverse `src/hooks/useImageViewer.js` orchestre, pour les modules « Drive », le routage « image → visionneuse, autre → ouverture classique » et fournit les props prêtes à étaler sur `ImageViewerModal`.

`src/lib/avatarStorage.js`, `src/lib/avatarCache.js` et `src/lib/imageCompress.js` (introduits en étape 16B) gèrent le pipeline complet des photos de profil. `avatarStorage` : wrappers Supabase Storage pour le bucket `avatars` (upload upsert sur path flat `{userId}.jpg`, delete, génération d'URL signée 2h). `avatarCache` : cache mémoire des URL signées avec TTL 1h, coalescence des Promises concurrentes (50 composants qui demandent le même avatar en même temps → un seul appel Supabase). `imageCompress` : compression canvas API native (pas de dépendance) vers JPEG carré avec dégradation de qualité par paliers si le résultat dépasse la taille cible. Le hook `src/hooks/useAvatarUpload.js` orchestre le pipeline complet (compress → upload → update DB → invalidate cache).
...
16. ✓ **Étape 16 — FAITE** (sous-étapes 16A → 16E) — Photos de profil
    ...
    - 16C : Composant `<Avatar>` centralisé dans `src/components/common/Avatar.jsx`. Affiche photo si `photo_url` présent (URL signée via cache), sinon initiales sur fond coloré déterministe (`getAvatarPalette`). Pendant chargement URL : initiales (pas de flash). Gestion `onError` du `<img>` pour fallback initiales si l'URL signée expire. Prop `alt` pour MemberAvatars (sans nom visible à côté). Taille proportionnelle (texte = 37% de la taille du cercle).
    - 16D : Modale d'upload + hook + intégration Profil. `src/components/common/AvatarUploadModal.jsx` : bottom-sheet 3 étapes via Portal (sélection input file natif → crop interactif avec `react-easy-crop` + slider zoom → confirmation preview ronde). `src/hooks/useAvatarUpload.js` : pipeline atomique (compress + upload + update DB + invalidate cache), avec gestion d'erreur best-effort sur le delete Storage. Intégration dans `src/pages/Profil.jsx` : avatar 108×108 + bouton "Modifier ma photo" + modale. Refacto de `useRole.js` : ajout de `photo_url` au retour, ajout d'une méthode `refetch()` pour rafraîchir après upload.
    - 16D-bis : Fix transverse des dépendances `useEffect`. Bug latent découvert pendant les tests : un hoquet réseau (ou un refresh JWT Supabase automatique) déclenchait une boucle de fetch infinie dans `useRole`, `useMedecin`, `useMedecins` parce que l'objet `user` du hook `useAuth` change de référence à chaque event d'auth. Fix : remplacer `[user, ...]` par `[user?.id, ...]` dans les dépendances (et `!user` par `!user?.id` dans les conditions internes). Pattern défensif à appliquer transversalement dans tout futur hook qui consomme `useAuth().user`.
    - 16E : Diffusion de `<Avatar>` dans tous les sites d'affichage (refacto pur, pas de nouvelle fonctionnalité). Sous-étapes :
      - 16E-1 : `MedecinCard` (Trombinoscope liste) — au passage, correction d'un bug latent où `<img src={photo_url}>` essayait de charger le path Storage comme URL directe (avant 16D, `photo_url` n'avait jamais de valeur, donc le bug était dormant).
      - 16E-2 : `MedecinDetail` + ajout du bouton "Modifier ma photo" (wording conditionnel "ma" / "la" selon que ce soit sa propre fiche ou non) + modale `AvatarUploadModal` montée. Visible uniquement en mode 'view' (pas en édition de formulaire) et conditionnée à `canEditMedecin`.
...
17. ✓ **Étape 17 — FAITE** (sous-étapes 17A → 17E + correctif iOS) — Notifications push Firebase FCM

Architecture retenue : **un seul service worker** (stratégie `injectManifest` de `vite-plugin-pwa`, en remplacement de `generateSW`) qui héberge à la fois le precache Workbox **et** le handler FCM en arrière-plan. `getToken` réutilise notre SW (paramètre `serviceWorkerRegistration`), donc **pas** de `firebase-messaging-sw.js` séparé. Ce montage est le pattern supporté et fonctionnel sur iOS (push iOS depuis 16.4, via APNs, sans compte Apple Developer ; le SW est bundlé en production donc OK sur Safari iOS).

   - **17A : Console Firebase + variables d'environnement.** Projet Firebase `omnes-orga`, app web, clé VAPID générée. 5 variables **client (non secrètes)** dans `.env` (gitignoré) puis sur Vercel : `VITE_FIREBASE_API_KEY`, `VITE_FIREBASE_PROJECT_ID`, `VITE_FIREBASE_MESSAGING_SENDER_ID`, `VITE_FIREBASE_APP_ID`, `VITE_FIREBASE_VAPID_KEY`. Piège rencontré : une valeur `.env` doit tenir sur **une seule ligne** (un saut de ligne collé par erreur corrompt le fichier).
   - **17B : Service worker unique.** `src/sw.js` (Workbox : `precacheAndRoute` + `cleanupOutdatedCaches`, `NavigationRoute` en fallback vers `index.html` avec denylist `[/^\/api/, /supabase\.co/, /\.(?:png|ico|svg|webmanifest)$/]`, cache Google Fonts, `skipWaiting` + `clientsClaim`). `vite.config.js` repassé en `strategies: 'injectManifest'` (`srcDir: 'src'`, `filename: 'sw.js'`, `devOptions.type: 'module'`, `globPatterns` étendus). 6 dépendances dev `workbox-*` en `^7.4.1` (alignées sur Workbox 7 de `vite-plugin-pwa`). Precache ~21 entrées (~1,4 Mo). Enregistrement auto par le plugin (`main.jsx` / `index.html` non touchés).
   - **17C : Client FCM.** Dépendance prod `firebase` (^12). `src/lib/firebase.js` : `initializeApp` + `getMessaging` gardé par `isSupported()`, expose `notificationsSupported()`, `requestNotificationToken()` (permission + `getToken` avec `vapidKey` et `serviceWorkerRegistration` via `navigator.serviceWorker.ready`), `onForegroundMessage()`. Handler ajouté dans `src/sw.js` (`firebase/app` + `firebase/messaging/sw`, `onBackgroundMessage` data-only → `showNotification` icône `/pwa-192x192.png`, plus listener `notificationclick` : focus du client existant ou `openWindow`). Hook `src/hooks/useNotifications.js` (états `supported` / `permission` / `enabling` / `error` ; `enableNotifications()` stocke le token dans `profiles.fcm_token`). Section Notifications dans `src/pages/Profil.jsx` (bouton « Activer les notifications », états détection / indisponible / autorisé / refusé). Affichage en premier plan (`onForegroundMessage`) volontairement **non branché** (optionnel ; le Realtime couvre déjà l'app ouverte).
   - **17D : Envoi côté serveur.** Le **seul secret réel** du projet : le JSON de compte de service Firebase, encodé en base64 et stocké en secret Supabase `FIREBASE_SERVICE_ACCOUNT_B64` (le fichier JSON n'est jamais commité). Edge Function `supabase/functions/send-notification/index.ts` (mêmes conventions que `create-medecin` : imports esm.sh dont `jose@5`, CORS, auth du caller via client anon + client `service_role`). Flux : authentifie l'appelant → lit `{ userIds, title, body, url }` → récupère les `fcm_token` → forge un access token Google (`jose` SignJWT RS256, scope `firebase.messaging`) → POST **data-only** vers l'API FCM v1 (`messages:send`, `webpush` Urgency high) → renvoie `{ success, sent, failed, recipients }`. Note : tout utilisateur authentifié peut l'appeler (acceptable en V1 pour un cabinet de confiance). Testée via `curl` → `{ "success": true, "sent": 1 }`.
   - **17E : Déclencheurs + production.** `src/lib/notify.js` expose `notifyUsers({ userIds, title, body, url })` — wrapper « fire-and-forget » autour de `supabase.functions.invoke('send-notification', …)`, filtre les ids vides, avale les erreurs silencieusement (une notif qui échoue ne doit jamais casser l'action métier). 4 déclencheurs, tous **après insertion réussie** et **excluant l'auteur** : message Discussion (`features/discussion/useCard.js`), message Immobilier (`features/immobilier/useCard.js`), création de tableau Discussion (`useDiscussion.js`), création de tableau Immobilier (`CreateBoardModal.jsx`). 5 variables ajoutées sur Vercel + redéploiement. **Test iPhone réel réussi** : PWA installée sur l'écran d'accueil, notifications activées, push délivré et affiché.
   - **Correctif iOS (post-test) :** sur iOS, une PWA en arrière-plan est « gelée » → la connexion Realtime se coupe et les messages manqués ne sont pas rechargés au retour (au tap sur la notif, le fil apparaissait périmé). Fix : un `useEffect` qui écoute `visibilitychange` et rappelle `refetch()` dès que `document.visibilityState === 'visible'`, ajouté dans `features/discussion/useCard.js` et `features/immobilier/useCard.js`. Bénéficie à tout retour au premier plan, pas seulement aux notifs.

18. ✓ **Étape 18 — FAITE** (sous-étapes 18A → 18C) — Socle « non-lus » : pastilles d'accueil + section « En attente »

Objectif : faire remonter sur la page d'accueil tout ce qui attend l'utilisateur, via un seul socle qui alimente à la fois les pastilles des tuiles et une section « En attente » sous la grille. Périmètre : messages non lus (Discussion + Immobilier), sondages Discussion non votés, et sondages de présence des événements à venir non répondus.

   - **18A : Socle SQL — fonction RPC `get_mon_activite()`.** Une seule fonction `SECURITY DEFINER` (search_path figé sur `public, pg_temp`, même pattern que `is_board_member`), qui renvoie pour `auth.uid()` une liste plate typée. Aucune table créée : on agrège les tables de suivi existantes (`discussion_card_reads`, `immobilier_card_reads`, `discussion_poll_votes`, `evenement_reponses`), rien à maintenir par trigger. Quatre branches en `union all` : (1) messages non lus Discussion et (2) Immobilier — sur cartes ouvertes et non archivées, dans un tableau non archivé où je suis membre, en comptant les messages des autres postérieurs à mon `last_read_at` de la carte (ou tous si je n'ai jamais ouvert la carte) ; (3) sondages Discussion ouverts (`closed = false`) sur carte ouverte où je n'ai pas de ligne de vote ; (4) sondages de présence d'événements avec `sondage_actif = true`, pas encore passés (`coalesce(date_fin, date_debut) >= current_date`) et sans réponse de ma part. Colonnes renvoyées : `item_type` (message/sondage), `module` (discussion/immobilier/evenements), `board_id`, `board_titre`, `card_id`, `evenement_id`, `couleur`, `titre`, `non_lus`, `ref_at`. La fonction normalise au passage les noms anglais Discussion vers une sortie unique (point de normalisation EN→FR centralisé). `execute` révoqué de `public`, accordé à `authenticated`. SQL versionné `docs/sql/18A-1-home-activite-rpc.sql`.
   - **18B : Hook `useMonActivite`** (`src/hooks/useMonActivite.js`). Appelle la RPC via `supabase.rpc('get_mon_activite')` et agrège côté client. Expose `{ items, parModule, loading, error, refetch }` : `items` = la liste triée (feed), `parModule` = compteurs par module sous la forme `{ nonLus, sondages, cartes, total }` (les 3 modules toujours initialisés à zéro pour que l'UI lise sans garde). `total = cartes + sondages` (nombre de choses distinctes à traiter). Conventions alignées sur les autres hooks : dépendance `user?.id` (et non `user`) pour éviter la boucle de fetch au refresh du JWT (leçon 16D-bis), `refetch` via `reloadKey`, et rechargement automatique au retour au premier plan via `visibilitychange` (pattern de l'étape 17, utile car la PWA iOS est gelée en arrière-plan).
   - **18C : UI (pastilles + section « En attente »).** `ModuleTile` supportait déjà une prop `badge` (pastille blanche en haut à droite, teintée de la couleur du module) : elle est simplement branchée depuis `Home.jsx` avec `badge = parModule[key].total` (mappée sur `discussion` / `immobilier` / `evenements` ; les autres tuiles n'ont pas de pastille). `ActivityList.jsx` est réécrit (il n'était qu'un placeholder statique) : il reçoit `items` + `loading` + `onSelect`, et affiche pour chaque ligne une pastille couleur module, le titre (carte / question de sondage / titre d'événement) et une ligne meta (« N nouveaux messages · Tableau », « Sondage à voter · … », « Sondage de présence à répondre »), le tout cliquable. `Home.jsx` gère la navigation : ligne Discussion/Immobilier → `/{module}/:boardId/:cardId`, ligne Événement → `/evenements/:id`. Titre de section « En attente », état vide « Rien en attente pour le moment. ». La liste ne clignote pas lors d'un refetch en arrière-plan (on garde l'affichage tant qu'on a déjà des items).

Décisions :

- Pastille = `total` (cartes avec du nouveau + sondages en attente), un compte « calme » et cohérent sur les 3 modules (pour Événements, c'est le nombre de sondages non répondus). Le hook expose aussi `nonLus` (nombre de messages) si l'on veut plus tard un style « WhatsApp » sur Discussion/Immobilier.
- Messages comptés sur cartes ouvertes et non archivées uniquement, et hors mes propres messages.
- Périmètre « à voter » = sondages Discussion + sondages de présence des événements à venir. Immobilier n'a pas de sondage ; un événement sans sondage ne remonte pas.
...
Fichiers modifiés : `src/pages/Home.jsx` (branche `useMonActivite`, pastilles, navigation du feed), `src/components/home/ActivityList.jsx` (placeholder → feed réel). `ModuleTile.jsx` inchangé (prop `badge` déjà présente).
   ...
   - ✓ **Sous-étape 18E — Retouches du système « non-lus » (3 corrections de finition).** Aucune migration SQL, aucune nouvelle infra : ces trois retouches réutilisent les briques existantes.

     - **18E-1 — La pastille d'icône s'efface dès la lecture.** Avant, la pastille n'était recalculée que par `useMonActivite`, monté uniquement sur la Home : il fallait remonter jusqu'à l'accueil pour la voir baisser. Nouveau helper `src/lib/appBadge.js` exposant `refreshAppBadge()` (appelle `get_mon_activite`, compte les lignes, pose/efface la pastille). Appelé juste après le marquage « lu » dans `markCardRead` (Discussion) et `markRead` (Immobilier), donc la pastille reflète l'état réel dès qu'on ouvre une carte, sans repasser par la Home. Feature-detection (no-op silencieux si l'API Badging est absente).

     - **18E-2 — Push (et donc pastille) à la création d'un sondage.** Avant, aucun push ne partait à la création d'un sondage (les déclencheurs de l'étape 17 ne couvraient que les messages et la création de tableau), donc l'icône ne se mettait pas à jour app fermée. Ajout de deux appels `notifyUsers` (fire-and-forget) : à la création d'un **sondage Discussion** (`usePoll.createPoll` → membres du tableau, hors créateur) et à la création d'un **événement avec sondage de présence** (`useEvenements.createEvenement` → tous les comptes actifs, hors créateur). La chaîne existante fait le reste : l'Edge Function recalcule `get_activite_count` par destinataire et le service worker pose la pastille. Décisions assumées : le sondage d'événement notifie **tout le cabinet** (c'est « en attente » pour tout le monde) ; le déclenchement se fait à la **création** (pas si on active le sondage en éditant un événement existant).

     - **18E-3 — Le clic sur une notification ouvre la bonne carte / le bon événement.** Avant, app en arrière-plan, le `notificationclick` faisait `focus()` sans naviguer (on restait sur la page courante / la Home) ; `client.navigate()` s'est révélé peu fiable sur iOS. Solution retenue : le service worker envoie un message `{ type: 'NAVIGATE', url }` à l'app ouverte (`client.postMessage`), et un écouteur monté dans `src/App.jsx` (`ServiceWorkerNavigation`, à l'intérieur du Router) navigue via React Router — **sans rechargement**, fiable sur iOS. App complètement fermée : `openWindow(url)` ouvre directement la bonne URL. Vaut pour les notifs de message comme de sondage.

     **Fichier ajouté :** `src/lib/appBadge.js`.
     **Fichiers modifiés :** `src/features/discussion/useCard.js` et `src/features/immobilier/useCard.js` (recalcul pastille après « lu »), `src/features/discussion/usePoll.js` et `src/features/evenements/useEvenements.js` (push à la création d'un sondage), `src/sw.js` (`notificationclick` → `postMessage`), `src/App.jsx` (écouteur `ServiceWorkerNavigation`).

     **Note multi-plateforme (Android) :** la pastille **numérique** d'icône repose sur l'API Badging, non supportée par Chromium sur Android — à la place, Android affiche automatiquement un **point** sur l'icône tant qu'une notification est non lue dans le volet (géré par l'OS). Les utilisateurs Android ont donc un point de présence plutôt qu'un chiffre ; le compteur exact reste visible dans l'app (tuiles + « En attente »). Nos appels `setAppBadge`/`clearAppBadge` y sont sans effet visible mais sans danger (feature-detection). Push, deep-link, installation et section « En attente » fonctionnent normalement sur Android (souvent mieux que sur iOS).

19. ✓ **Étape 19 — FAITE** (sous-étapes 19A → 19D) — Visionneuse d'image intégrée (pièces jointes & documents)

Problème de départ : cliquer sur une photo en pièce jointe se comportait mal une fois l'app déployée. Sur iPhone en PWA installée, l'image sortait de l'application via la feuille de partage iOS (Aperçu) ; sur Android, les images (testé en JPEG) ne s'ouvraient pas du tout. Les PDF, eux, s'ouvraient correctement sur les deux systèmes.

Cause : depuis l'étape 15 bis, toutes les pièces jointes passaient par `openOrDownload`, qui tente d'**ouvrir** le fichier (nouvel onglet ou téléchargement). Or ces deux voies sont bloquées ou peu fiables sur mobile pour une image.

Solution retenue : pour une image, on ne l'**ouvre** plus — on l'**affiche directement dans l'application**, dans une visionneuse plein écran (un simple `<img>` avec une URL signée). Ce n'est ni un `window.open()` ni un téléchargement, donc les blocages iOS PWA et Android ne s'appliquent pas : un clic, l'image reste dans l'app, sur les deux plateformes. Les autres types (PDF, Word, Excel…) gardent leur comportement existant via `openOrDownload`.

   - **19A : Fondations transverses.** Composant `src/components/common/ImageViewerModal.jsx` — visionneuse plein écran via React Portal (overlay sombre, fermeture au tap/croix/Échap, verrouillage du scroll du body). Zoom maison **sans dépendance externe** : double-tap / double-clic pour basculer x1 / x2.5, pincer pour zoomer finement, glisser pour déplacer quand c'est zoomé (utile pour lire une ordonnance ou un résultat photographié). Filet de sécurité : un bouton « Télécharger » s'affiche si l'image ne peut pas être rendue par le navigateur (cas HEIC sur Android, cf. limitations). Dans `src/lib/storageOpen.js` : ajout de `isImage(mimeType)` et de `createSignedImageUrl({ supabase, bucket, storagePath, expiresIn })` (URL signée d'un bucket privé pour le `src` de l'image).
   - **19B : Immobilier.** La chip ouvrant elle-même les fichiers, le branchement est dans `AttachmentChip.jsx` : si l'attachement est une image → visionneuse ; sinon → `openAttachment` (inchangé). Ajout dans `immobilierStorage.js` de `getAttachmentImageUrl(attachment)` et `downloadAttachment(attachment)` (repli Blob pour le bouton « Télécharger » de la visionneuse).
   - **19C : Discussion.** La chip déléguant l'ouverture via `onOpen`, le branchement est dans la section `CardAttachments.jsx` (la chip n'est pas touchée, l'ouverture des PDF/Word continue de passer par le parent). Ajout dans `discussionStorage.js` de `getAttachmentImageUrl(storagePath)` (le repli téléchargement réutilise `downloadAttachment` déjà présent).
   - **19D : Cabinet pratique, SIM, Événements.** Mutualisé via un hook partagé `src/hooks/useImageViewer.js` : il expose `openFile(id, nom, mimeType)` (image → visionneuse ; sinon → repli) et `viewerProps` (à étaler sur `ImageViewerModal`). Pour Cabinet et SIM, le branchement est dans le composant `DrivePage` de chaque module (les pages `Cabinet.jsx` / `CabinetFolder.jsx` / `Sim.jsx` / `SimFolder.jsx` ne sont **pas** modifiées) : le hook reçoit `onOpenFile` comme repli et `onDownloadFile` comme téléchargement, ce qui préserve la gestion d'erreur existante des pages. Pour Événements, branchement direct dans `EvenementDocuments.jsx`. Ajout d'un `getCabinetImageUrl` / `getSimImageUrl` / `getEvenementImageUrl` dans les helpers Storage respectifs.

Décisions :
- La visionneuse s'applique à **toutes les images de l'app** (pièces jointes de chat + documents des Drive), sur toutes les plateformes (y compris Mac), pour un comportement uniforme.
- Point d'injection choisi au plus près de l'existant module par module (chip pour Immobilier, section pour Discussion, `DrivePage` pour les Drive), ce qui minimise les fichiers touchés et préserve les comportements en place.
- Zoom volontairement minimal mais suffisant pour l'usage médical (lire un document photographié).
...
- **Immobilier : deux vues des participants** — depuis le 24/09/2026, taper la pile d'avatars ouvre `MembersSheet` (lecture seule, coup d'œil), tandis que le menu trois-points garde `ManageMembersModal` (inviter / retirer / quitter). Répartition volontaire : consultation d'un côté, action de l'autre. Discussion n'a que la première, le module n'ayant pas d'écran de gestion des membres. À revoir si l'usage montre que les médecins cherchent la gestion en tapant les avatars.
- **Badge « Créateur » : libellé unique, sémantique à deux vitesses** — le badge marque `discussion_board_members.role = 'owner'` (Discussion) et `immobilier_board_members.role_in_board = 'owner'` (Immobilier). Dans les deux cas il s'agit en pratique du créateur du tableau, et d'une seule personne (cf. `MembersSheet` dans les composants transverses). Les droits attachés diffèrent pourtant : côté Discussion le créateur peut renommer et archiver (via `board.created_by`, pas via le rôle de membre, qui est purement descriptif) ; côté Immobilier il peut en plus inviter, retirer et éditer les cartes. Un même mot pour deux périmètres — acceptable tant que le créateur reste unique, à réexaminer si on ouvre un jour la promotion en owner.
- **Cohérence du nommage Discussion** — le module Discussion utilise des noms anglais (`title`, `status`, `archived`, `created_by`) en BDD, hooks et composants, alors qu'Immobilier utilise le français (`titre`, `statut`, `archive`, `auteur_id`). Cette dette de nommage transverse rend la lecture du code plus pénible (helpers de normalisation en transit dans `Recherche.jsx`) mais n'a pas d'impact utilisateur. Renommage prévu en étape 12 ter ou après le déploiement Vercel : impacte les colonnes Postgres, les RLS, les fonctions SECURITY DEFINER (`is_board_member`, `is_board_owner`, `mark_board_read`), les hooks (`useDiscussion`, `useBoard`, `useCard`), les composants et la doc. Sous-chantiers prévus : (a) SQL + RLS + fonctions, (b) hooks JS, (c) composants et UI, (d) doc et limitations. Assaini partiellement le 24/09/2026 côté **texte affiché** d'Immobilier : `ManageMembersModal` montrait « Owner » en anglais et quatre messages non accentués (« Desinviter », « seul owner », « Vous etes l'unique owner », `super_admin` affiché brut). Ces libellés sont passés en français accentué et le badge fait main (ocre sur `bg-ocre/10`, entorse au DS qui impose `ocre-fonce` sur ce fond pastel) a été remplacé par le composant `Pill` en canard, identique à celui de `MembersSheet`. Les identifiants de code et les valeurs en base (`owner`, `role_in_board`) sont **inchangés** — c'est bien le renommage de fond qui reste à faire.
...
- **Images : visionneuse intégrée — RÉSOLU (étape 19)** — depuis l'étape 19, cliquer sur une image (pièce jointe de chat ou document d'un Drive) l'affiche dans une visionneuse plein écran **dans l'application**, sur iOS comme sur Android, sans passer par la feuille de partage iOS ni le téléchargement. Le souci d'origine (image sortant de l'app sur iPhone, ne s'ouvrant pas sur Android) est levé pour les images. Les PDF et les autres types continuent de passer par `openOrDownload` (ouverture native / téléchargement), comportement inchangé.
- **HEIC non rendu par certains navigateurs** — une photo iPhone au format HEIC (`image/heic`) peut ne pas s'afficher dans un `<img>`, en particulier sur Android/Chrome (Safari, lui, sait l'afficher). Dans ce cas, la visionneuse détecte l'échec de chargement et propose un bouton « Télécharger » en repli (le fichier s'ouvre alors via l'application native du téléphone). En pratique, l'envoi d'une photo depuis l'iPhone produit souvent un JPEG selon les réglages iOS, donc le cas est rare.
